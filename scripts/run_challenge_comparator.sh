...
#   bash scripts/run_challenge_comparator.sh --config comparator/candidate-01-donsker-varadhan.json
#   bash scripts/run_challenge_comparator.sh --config comparator/pending-kl-chain-rule.json
...
# Challenge/README.md for the manifest. The five DRSB source papers and the Drsb
# capstone are documented, not comparator challenges. `TiltedKernel` and
# `BirkhoffHopf` are Leaderboard-only (their statements name project definitions)
# and so have no comparator config.
#
...
# The failure mode is documented in aiq-dkps-formalization:
...
DEFAULT_CONFIGS=(
    "comparator/candidate-01-donsker-varadhan.json"
    "comparator/candidate-02-kl-convexity.json"
    "comparator/candidate-03-supergradient.json"
    "comparator/pending-gibbs-tilt.json"
    "comparator/pending-kl-chain-rule.json"
    "comparator/pending-kl-lsc.json"
    "comparator/pending-measurable-argmax.json"
    "comparator/pending-integrable-integral-compprod.json"
)
