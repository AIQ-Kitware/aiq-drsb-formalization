...
/-- General KL-exhaustion theorem target from dyadic generation.

This is the reusable information-theory capstone that should eventually replace the explicit
`HasIntervalDyadicKLExhaustion` interface: for a path measurable space generated by the dyadic
projection filtration, projected relative entropies converge to the full path-space relative
entropy. -/
theorem hasIntervalDyadicKLExhaustion_of_dyadicGeneration
    (W : ProbabilityMeasure IntervalPath)
    (_hgen : HasIntervalDyadicGeneration (inferInstance : MeasurableSpace IntervalPath)) :
