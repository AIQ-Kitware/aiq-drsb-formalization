  ...
  have havg_eq_diff : avg = diff := by
    ...
    calc
      ...
      _   = R - (1 / (n : ℝ)) * (∑ i : Fin n, g (s i)) := by
              ...
              field_simp [hnne]
      _   = diff s := by
              simp [diff, empMean, sub_eq_add_neg]
...
theorem pacBayes_generalization_bound
  ...
  have hbad_meas : MeasurableSet bad := by
    classical
    -- `r` is measurable as a finite sum of measurable coordinate maps.
    have hr_meas : Measurable r := by
      dsimp [r, empMean]
      unfold empMean
      measurability
    have hdiff_meas : Measurable (fun s : Fin n → X => R - r s) :=
      measurable_const.sub hr_meas
    have hIci : MeasurableSet (Set.Ici thr) := isClosed_Ici.measurableSet
    -- `bad = (R - r) ⁻¹' Ici thr`
    simpa [bad, Set.preimage, Set.mem_Ici] using hIci.preimage hdiff_meas
...
/-- Extended-valued KL divergence `ℝ≥0∞` (Mathlib: `InformationTheory.klDiv`). -/
noncomputable abbrev KLinf {Ω : Type*} [MeasurableSpace Ω] (P Q : Measure Ω) : ℝ≥0∞ :=
  InformationTheory.klDiv P Q

/-- Real KL used in the paper (guard with finiteness!). -/
noncomputable abbrev KL {Ω : Type*} [MeasurableSpace Ω] (P Q : Measure Ω) : ℝ :=
  (KLinf (P := P) (Q := Q)).toReal

/-- KL finiteness predicate (prevents `toReal ⊤ = 0`). -/
def KLFinite {Ω : Type*} [MeasurableSpace Ω] (P Q : Measure Ω) : Prop :=
  KLinf (P := P) (Q := Q) ≠ ⊤

lemma KL_nonneg {Ω : Type*} [MeasurableSpace Ω] (P Q : Measure Ω) : 0 ≤ KL (P := P) (Q := Q) := by
  simp [KL, KLinf, ENNReal.toReal_nonneg]


/-- Under finiteness, `ENNReal.ofReal (KL P Q) = KLinf P Q`. -/
lemma ofReal_KL (P Q : Measure Ω) (h : KLFinite (P := P) (Q := Q)) :
    ENNReal.ofReal (KL (P := P) (Q := Q)) = KLinf (P := P) (Q := Q) := by
  simpa [KL, KLFinite, KLinf] using ENNReal.ofReal_toReal h

/--
Donsker–Varadhan / entropy inequality (the direction used in the paper step (121)):

This is the “change of measure” step that makes `KL(P‖Q)` appear.
You can state it either with `Real.log (∫ exp f dQ)` or in an exponentiated form.
-/
theorem donsker_varadhan_ineq
    {Ω : Type*} [MeasurableSpace Ω]
    (P Q : Measure Ω) [IsProbabilityMeasure P] [IsProbabilityMeasure Q]
    (f : Ω → ℝ) (hf : Measurable f)
    (hKL : KLFinite (P := P) (Q := Q))
    (hexp_int : Integrable (fun ω => Real.exp (f ω)) Q) :
    (∫ ω, f ω ∂P) ≤ KL (P := P) (Q := Q) + Real.log (∫ ω, Real.exp (f ω) ∂Q) := by
  -- This is where Aristotle proves DV properly.
  -- It should follow from RN derivative + Jensen (log) + change of variables.
  sorry
...
theorem theorem4_paths
  ...
  have hk : 0 ≤ KL (P := P) (Q := Q) := by
    -- Nonnegativity holds unconditionally. We keep `hKL` as an explicit hypothesis
    -- to match the paper's intent that the KL term is *finite*.
    simpa using KL_nonneg (P := P) (Q := Q)

  haveI : IsProbabilityMeasure P₁ := by
    simpa [P₁, terminalMarginal] using
      terminalMarginal_isProbability (P := P) (X1 := eval₁) heval₁
...
namespace RevisedPaper


/-!
A faithful scaffold of the *revised* 2-pager proof:

- Randomness is over `S` (a probability measure on `Θ`).
- `r θ x` is the (random) empirical risk.
- `R x = E_{θ~S}[r θ x]` is the true risk, so `R - r` is centered under `S`.
- `Q` is the reference path measure (prior).
- `P` is a controlled path measure (posterior), allowed to depend on `θ`.
- KL is Mathlib’s `InformationTheory.klDiv`.

We isolate the hard steps as lemma stubs:
  * Lemma 3 (Hoeffding mgf bound for bounded centered variables)
  * Fubini swap
  * Lemma 5 (Donsker–Varadhan variational formula, *restricted to finite-KL posteriors*)
  * Chernoff/Markov step
-/

variable {Ω Θ X : Type*} [MeasurableSpace Ω] [MeasurableSpace Θ] [MeasurableSpace X]

/-- Terminal marginal of a path measure via a measurable terminal map `X1`. -/
noncomputable def terminalMarginal (X1 : Ω → X) (P : Measure Ω) : Measure X :=
  Measure.map X1 P

/-- Extended KL divergence `ℝ≥0∞` from Mathlib. -/
noncomputable abbrev KLinf (P Q : Measure Ω) : ℝ≥0∞ :=
  InformationTheory.klDiv P Q

/-- Finite-KL predicate (prevents `toReal ⊤ = 0`). -/
def KLFinite (P Q : Measure Ω) : Prop :=
  KLinf (P := P) (Q := Q) ≠ ⊤

/-- Real KL used in the bound (only meaningful under `KLFinite`). -/
noncomputable abbrev KL (P Q : Measure Ω) : ℝ :=
  (KLinf (P := P) (Q := Q)).toReal

lemma KL_nonneg (P Q : Measure Ω) : 0 ≤ KL (P := P) (Q := Q) := by
  simp [KL, KLinf, ENNReal.toReal_nonneg]

/-- Type of posteriors we allow in the DV supremum: probability measures with finite KL to `Q`. -/
def FinPost (Q : Measure Ω) :=
  { P : Measure Ω // IsProbabilityMeasure P ∧ KLFinite (P := P) (Q := Q) }


noncomputable def dvSup (Q : Measure Ω) (f : Ω → ℝ) : ℝ :=
  sSup (Set.range (fun P : FinPost (Q := Q) =>
    (∫ ω, f ω ∂(P.1)) - KL (P := P.1) (Q := Q)))

/-!
### Lemma stubs matching the revised proof steps
-/

/-- (Lemma 3 in the revised paper) Hoeffding mgf bound for centered bounded RV. -/
theorem lemma3_hoeffding_mgf
    (S : Measure Θ) [IsProbabilityMeasure S]
    (U : Θ → ℝ) (hU_meas : Measurable U)
    (a : ℝ) (ha : 0 ≤ a)
    (hU_bd : ∀ θ, U θ ∈ Set.Icc (-a) a)
    (hU_mean0 : (∫ θ, U θ ∂S) = 0)
    (t : ℝ) :
    (∫ θ, Real.exp (t * U θ) ∂S) ≤ Real.exp (t^2 * a^2 / 2) := by
  sorry

/-- Fubini swap used in (120): `E_Q E_S = E_S E_Q` for our integrand. -/
theorem fubini_swap
    (S : Measure Θ) [IsProbabilityMeasure S]
    (Q : Measure Ω) [IsProbabilityMeasure Q]
    (h : Θ → Ω → ℝ)
    (hh_meas : Measurable (fun p : Θ × Ω => h p.1 p.2))
    (hint : Integrable (fun p : Θ × Ω => h p.1 p.2) (S.prod Q)) :
    (∫ ω, (∫ θ, h θ ω ∂S) ∂Q) = (∫ θ, (∫ ω, h θ ω ∂Q) ∂S) := by
  sorry

/-- (Lemma 5 in the revised paper) DV variational formula, restricted to finite-KL posteriors. -/
theorem lemma5_dv
    (Q : Measure Ω) [IsProbabilityMeasure Q]
    (f : Ω → ℝ) (hf : Measurable f)
    (hbdd : BddAbove (Set.range (fun P : FinPost (Q := Q) =>
      (∫ ω, f ω ∂(P.1)) - KL (P := P.1) (Q := Q))))
    (hexp : Integrable (fun ω => Real.exp (f ω)) Q) :
    Real.log (∫ ω, Real.exp (f ω) ∂Q) = dvSup (Q := Q) f := by
  sorry

/-- Chernoff/Markov step used to go from `E[exp Z] ≤ 1` to a tail probability bound. -/
theorem chernoff_from_exp_le_one
    (S : Measure Θ) [IsProbabilityMeasure S]
    (Z : Θ → ℝ) (hZ_meas : Measurable Z)
    (hZ_int : Integrable (fun θ => Real.exp (Z θ)) S)
    (hEZ : (∫ θ, Real.exp (Z θ) ∂S) ≤ 1)
    {ϱ : ℝ} (hϱ : 0 < ϱ) :
    S {θ | Z θ > Real.log (1 / ϱ)} ≤ ENNReal.ofReal ϱ := by
  sorry

/-!
### The revised Theorem 4 (Eq. (126) style)

This matches the revised proof structure: randomness is over `S`,
posterior `P` may depend on `θ`, and the bound includes `KL(P‖Q)` and `2 ε C^2`.
-/

/-- Revised-paper PAC-Bayes bound (faithful to the revised proof’s intent). -/
theorem theorem4_revised
    (S : Measure Θ) [IsProbabilityMeasure S]
    (Q : Measure Ω) [IsProbabilityMeasure Q]
    (X1 : Ω → X) (hX1 : Measurable X1)
    (r : Θ → X → ℝ)
    (hr_meas : Measurable (fun p : Θ × X => r p.1 p.2))
    (C : ℝ) (hC : 0 ≤ C)
    (hr_bd : ∀ θ x, |r θ x| ≤ C)
    (post : Θ → FinPost (Q := Q))
    {ε ϱ : ℝ} (hε : 0 < ε) (hϱ : 0 < ϱ ∧ ϱ < 1) :
    let R : X → ℝ := fun x => ∫ θ, r θ x ∂S
    -- “with probability ≥ 1-ϱ over θ~S”
    S {θ |
      let P : Measure Ω := (post θ).1
      let P₁ : Measure X := terminalMarginal (X1 := X1) P
      (∫ x, R x ∂P₁)
        ≤ (∫ x, r θ x ∂P₁)
          + (KL (P := P) (Q := Q) + Real.log (1 / ϱ)) / ε
          + 2 * ε * C^2 } ≥ ENNReal.ofReal (1 - ϱ) := by
  intro R
  /-
  Proof outline matching revised PDF (118)-(126):

  1) Define Uθ(ω) := R(X1 ω) - r θ (X1 ω).
     Show Uθ is centered over θ~S for each ω (by definition of R as E_S[r]).
     Show bounds: Uθ(ω) ∈ [-2C,2C] using hr_bd.

  2) Apply lemma3_hoeffding_mgf pointwise in ω with a:=2C, t:=ε to obtain (118):
        E_S exp(ε (R-r)) ≤ exp(2 ε^2 C^2).

  3) Integrate over ω~Q, then swap with Fubini (119)-(120).

  4) Apply DV lemma (lemma5_dv) to identify log E_Q exp(ε(R-r)) with dvSup(Q, ε(R-r))
     (this is the “sup_P E_P[ε(R-r)] - KL(P‖Q)” step).

  5) Rearrange into an `E_S[exp(Z)] ≤ 1` form (122), then apply Chernoff/Markov
     (chernoff_from_exp_le_one) with threshold log(1/ϱ) to get a tail bound.

  6) Convert the tail event into the final inequality (126)-style by algebra.
  -/
  sorry
