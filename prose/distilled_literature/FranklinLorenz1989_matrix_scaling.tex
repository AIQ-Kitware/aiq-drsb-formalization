...
$\vartheta(A)$.  With the logarithmic convention above, the clean modern reading is
...
formula used in Lemma 1 is the multiplicative one, because the contraction coefficient
is
\[
  \kappa(A)
...
\section*{How this paper maps to the Lean projective-lag frontier}
The most useful theorem shape for formalization is not the entrywise bound by itself.
...
\]
If a finite phase stream in Lean is identified with one of these correction vectors,
...
\paragraph{Two formalization routes.}
One can either formalize Franklin--Lorenz close to the paper by defining
...
The latter is closer to the existing Lean projective-lag files; the former is closer to
