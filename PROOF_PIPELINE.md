...
|---|---|---|---|
| `ForMathlib.MeasureTheory.toReal_klDiv_eq_sSup_dvDualSet` | ✅ **PROVED** (dependency-clean) | — | **Donsker–Varadhan *dual* variational formula** — `KL(μ‖ν) = sup_f (∫f dμ − log ∫eᶠdν)` over bounded measurable `f`. A genuine Mathlib gap, and **not** the Gibbs formula we already had (`isGreatest_donskerVaradhan` sups over *measures* and attains; this sups over *functions* and does not). `≥` half = the old DV inequality; the new content is achievability via `truncLLR`, which puts `-n` (not `0`) on `{dμ/dν = 0}` because Lean's `Real.log 0 = 0` makes `exp (llr) = 1` there. Corollary `toReal_klDiv_le_of_tendsto_integral` = **setwise lsc of `klDiv`**, i.e. `KL`-balls are setwise closed — the entropic half of the `hattain` route. Upstreamable. |
