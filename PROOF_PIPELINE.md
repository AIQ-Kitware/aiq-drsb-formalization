...
| `Drsb.sdrsb_cost_bound` | ✅ **PROVED** (dependency-clean, **attainment-free**) | — | **The SDRSB evaluation-card claim, discharged.** Composes the proved `WangGaoXie2023.sinkhorn_weak_duality_kernel` (per-point DV bound integrated over `p₀`) with a *near-optimal* budget (`≤ ε + η`) + `η ↓ 0`, under `hκ>0` + the edge `hSink : ∀ η>0, IsSinkhornWitness … (ε+η)`. Attainment content gone (same `sInf` argument as WDRSB). `sdrsb_cost_bound_of_attained_witness` is the receipt that the old attained edge implies the new one. Dual restricted to `0<lam` (the `lam=0` `logPartition` is junk — ess-sup convention unencoded). Ball uses the audit-fixed external `ν`. |
| `Drsb.isSinkhornWitness_of_coupling` | ✅ **PROVED** (dependency-clean) | — | **The SDRSB disintegration edge, discharged.** On `[StandardBorelSpace X] [Nonempty X]`, a coupling `γ ∈ Π(p₀,μ)` with `γ ≪ p₀⊗ν` and `klDiv γ (p₀⊗ν) ≠ ⊤` *builds* the conditional family: `Measure.condKernel` + `Measure.integral_compProd` + the chain rule `toReal_klDiv_compProd_eq_integral` at `η := Kernel.const X ν`. The a.e. slice facts (`P x ≪ ν`, integrable `llr`) come from `klDiv_ne_top_iff` on the a.e.-finite slices (`ae_klDiv_kernel_ne_top`). **Prerequisite:** `sinkhorn_weak_duality_kernel`/`IsSinkhornWitness` weakened from `∀ x` to `∀ᵐ x ∂p₀` — a `condKernel` is only determined a.e. |
| `Drsb.sdrsb_cost_bound_of_plans` | ✅ **PROVED** (dependency-clean) | — | **The SDRSB card claim from transport plans alone** — no conditional family, no attainment. `hplan` (a near-optimal *finite-entropy* coupling for each `η>0`) is **not** derivable from `μ ∈ sinkhornBall`, because `klReal` is `toReal` and scores a singular coupling `0` (proved: `ForMathlib.OT.klReal_eq_zero_of_not_absolutelyContinuous`). Making `Wkappa` `ℝ≥0∞`-valued turns `hplan` into a theorem — the next definitional cleanup, see `STATUS.md`. |
