/-
# Birkhoff--Hopf projective contraction: shared neutral vocabulary

Route-neutral projective-kernel definitions and elementary lemmas shared by both finite
Birkhoff--Hopf developments in this repository:

* `Direct` — the AI-discovered Doeblin / weighted-average contraction proof;
* `PaperRoute` — the source-faithful Eveson--Nussbaum development.

This module deliberately imports **neither** proof route.  It fixes the common language
(strictly positive finite kernel, Hilbert projective log-spread, cross-ratio and its finite
bound, the coarse Birkhoff coefficient, and the contraction-coefficient predicates) together
with the positivity / bound / finite-`sSup` bookkeeping both routes build on.  Keeping it
route-neutral is what lets the paper route depend on this file without transitively importing
the direct proof.
-/
