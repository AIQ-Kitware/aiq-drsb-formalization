...
/-! ## The Kantorovich layer

Couplings, transport costs and the optimal-transport cost live between **two** measurable spaces
`α` and `β`, under a cost `c : α → β → ℝ`, with no algebraic or topological structure on either.
That is the shape Mathlib wants, and it is the shape the theory has: nothing in
`couplings`/`couplingCost`/`otCost` refers to a norm, to subtraction, or to `α = β`.

Only the **quadratic** specializations (`couplingCost2`, `W2sq`, `wassersteinBall`) need a single
normed group, and they are collected in their own section below. -/

variable {α β : Type*} [MeasurableSpace α] [MeasurableSpace β]

/-- Expectation `𝔼_μ[f] = ∫ f dμ` for a probability measure `μ` and real `f`. -/
noncomputable def expect (μ : ProbabilityMeasure α) (f : α → ℝ) : ℝ :=
  ∫ x, f x ∂(μ : Measure α)
...
/-- The set of couplings `Π(μ, ν)`: probability measures on `α × β` with first
marginal `μ` and second marginal `ν`. -/
def couplings (μ : ProbabilityMeasure α) (ν : ProbabilityMeasure β) :
    Set (ProbabilityMeasure (α × β)) :=
  { π | Measure.map Prod.fst (π : Measure (α × β)) = (μ : Measure α)
      ∧ Measure.map Prod.snd (π : Measure (α × β)) = (ν : Measure β) }

/-- Transport cost of a coupling under a (measurable) cost `c : α → β → ℝ`:
`𝔼_π[c(x, y)]`. -/
noncomputable def couplingCost (c : α → β → ℝ) (π : ProbabilityMeasure (α × β)) : ℝ :=
  ∫ z, c z.1 z.2 ∂(π : Measure (α × β))
...
general `c`. -/
noncomputable def otCost (c : α → β → ℝ) (μ : ProbabilityMeasure α) (ν : ProbabilityMeasure β) :
    ℝ :=
  sInf { r : ℝ | ∃ π : ProbabilityMeasure (α × β),
    π ∈ couplings μ ν ∧ r = couplingCost c π }

/-- Product measure `μ ⊗ ν` on `α × β`. -/
noncomputable def prodMeasure (μ : ProbabilityMeasure α) (ν : ProbabilityMeasure β) :
    Measure (α × β) :=
  (μ : Measure α).prod (ν : Measure β)
...
noncomputable def couplingCostENN (c : α → β → ℝ) (π : ProbabilityMeasure (α × β)) : ℝ≥0∞ :=
  ∫⁻ z, ENNReal.ofReal (c z.1 z.2) ∂(π : Measure (α × β))

/-- **Sinkhorn objective, `ℝ≥0∞`-valued** — `𝔼_γ[c(x,y)] + κ·KL(γ ‖ μ̂⊗ν)` with no `toReal`.
A singular `γ` scores `⊤` here, as it must. -/
noncomputable def sinkhornObjectiveENN (c : α → β → ℝ) (κ : ℝ) (μhat : ProbabilityMeasure α)
    (νref : ProbabilityMeasure β) (γ : ProbabilityMeasure (α × β)) : ℝ≥0∞ :=
  couplingCostENN c γ
    + ENNReal.ofReal κ * InformationTheory.klDiv (γ : Measure (α × β)) (prodMeasure μhat νref)
...
noncomputable def Wkappa (c : α → β → ℝ) (κ : ℝ) (νref : ProbabilityMeasure β)
    (μhat : ProbabilityMeasure α) (μ : ProbabilityMeasure β) : ℝ≥0∞ :=
...
def sinkhornBall (c : α → β → ℝ) (μhat : ProbabilityMeasure α) (νref : ProbabilityMeasure β)
    (κ ε : ℝ) : Set (ProbabilityMeasure β) :=
...
`sup_{μ ∈ 𝓐} 𝔼_μ[f]`. -/
noncomputable def droValue (𝓐 : Set (ProbabilityMeasure α)) (f : α → ℝ) : ℝ :=
  sSup { r : ℝ | ∃ μ : ProbabilityMeasure α, μ ∈ 𝓐 ∧ r = expect μ f }

/-! ## The quadratic (Wasserstein-2) specializations

These, and only these, need a single normed group. -/
