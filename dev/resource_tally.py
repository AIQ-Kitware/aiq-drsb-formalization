...
DESIGN (the constraints this satisfies)
    ...
    * Minimal agent tokens: the agent runs ONE command (or nothing, via the
      post-commit hook in dev/hooks/). Language-agnostic: a Codex agent can pass
      `--transcript <path>` to its own log.

USAGE
    python dev/resource_tally.py record            # attribute new turns -> HEAD
    python dev/resource_tally.py record --commit <sha>
    python dev/resource_tally.py rollup            # refresh lifetime totals in yaml
    python dev/resource_tally.py show              # print the ledger
    python dev/resource_tally.py reconcile         # catch un-recorded trailing turns
...
# ------------------------------------------------------------------------ ledger
def ledger_path() -> str:
    return os.path.join(repo_root(), "dev", "resource-ledger.jsonl")
...
def cmd_rollup(args) -> None:
    ...
    _write_yaml_totals(totals)
    print(json.dumps(totals, indent=2, ensure_ascii=False))


def _write_yaml_totals(totals: dict) -> None:
    """Replace the marked lifetime_totals region in formalization.yaml (no yaml dep)."""
    yml = os.path.join(repo_root(), "formalization.yaml")
    if not os.path.exists(yml):
        return
    # Path-free marker so relocating the tool never orphans the region.
    begin = "    # BEGIN lifetime_totals (auto-generated by `dev/resource_tally.py rollup`; do not edit by hand)"
    ...
    if begin in text and end in text:
        ...
        text = pre + "\n".join(block) + post
    else:  # append a resource_accounting-adjacent block if markers absent
        text = text.rstrip() + "\n\n" + "\n".join(block) + "\n"
