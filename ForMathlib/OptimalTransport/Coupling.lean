...
theorem integrable_of_couplingCostENN_ne_top (c : X → X → ℝ) (hc : ∀ x y, 0 ≤ c x y)
    (hcm : Measurable fun z : X × X => c z.1 z.2)
    (π : ProbabilityMeasure (X × X)) (h : couplingCostENN c π ≠ ⊤) :
    Integrable (fun z : X × X => c z.1 z.2) (π : Measure (X × X)) := by
  refine ⟨hcm.aestronglyMeasurable, ?_⟩
  have hcongr : ∫⁻ z : X × X, ‖c z.1 z.2‖ₑ ∂(π : Measure (X × X)) = couplingCostENN c π :=
...
theorem couplingCost_eq_toReal (c : X → X → ℝ) (hc : ∀ x y, 0 ≤ c x y)
    (hcm : Measurable fun z : X × X => c z.1 z.2) (π : ProbabilityMeasure (X × X)) :
    couplingCost c π = (couplingCostENN c π).toReal := by
  rw [couplingCost,
    integral_eq_lintegral_of_nonneg_ae (f := fun z : X × X => c z.1 z.2)
      (Filter.Eventually.of_forall fun z => hc _ _) hcm.aestronglyMeasurable]
  rfl

omit [NormedAddCommGroup X] in
/-- Converse of `integrable_of_couplingCostENN_ne_top`. -/
theorem couplingCostENN_ne_top_of_integrable (c : X → X → ℝ) (hc : ∀ x y, 0 ≤ c x y)
    (π : ProbabilityMeasure (X × X))
    (h : Integrable (fun z : X × X => c z.1 z.2) (π : Measure (X × X))) :
...
theorem couplingCostENN_ne_top_of_sinkhornObjectiveENN_ne_top {c : X → X → ℝ} {κ : ℝ}
...
theorem klDivMul_ne_top_of_sinkhornObjectiveENN_ne_top {c : X → X → ℝ} {κ : ℝ}
    {μhat νref : ProbabilityMeasure X} {γ : ProbabilityMeasure (X × X)}
...
still be singular. -/
theorem klDiv_ne_top_of_sinkhornObjectiveENN_ne_top {c : X → X → ℝ} {κ : ℝ} (hκ : 0 < κ)
    {μhat νref : ProbabilityMeasure X} {γ : ProbabilityMeasure (X × X)}
    (h : sinkhornObjectiveENN c κ μhat νref γ ≠ ⊤) :
    InformationTheory.klDiv (γ : Measure (X × X)) (prodMeasure μhat νref) ≠ ⊤ := by
...
theorem absolutelyContinuous_of_sinkhornObjectiveENN_ne_top {c : X → X → ℝ} {κ : ℝ} (hκ : 0 < κ)
    {μhat νref : ProbabilityMeasure X} {γ : ProbabilityMeasure (X × X)}
    (h : sinkhornObjectiveENN c κ μhat νref γ ≠ ⊤) :
...
theorem sinkhornObjective_eq_toReal_of_ne_top {c : X → X → ℝ} (hc : ∀ x y, 0 ≤ c x y)
    (hcm : Measurable fun z : X × X => c z.1 z.2) {κ : ℝ} (hκ : 0 ≤ κ)
    {μhat νref : ProbabilityMeasure X} {γ : ProbabilityMeasure (X × X)}
    (h : sinkhornObjectiveENN c κ μhat νref γ ≠ ⊤) :
...
theorem Wkappa_le_ofReal_of_mem_sinkhornBall {c : X → X → ℝ} {p₀ ν μ : ProbabilityMeasure X}
...
theorem radius_nonneg_of_mem_sinkhornBall {c : X → X → ℝ} {p₀ ν μ : ProbabilityMeasure X}
...
theorem exists_coupling_sinkhornObjectiveENN_lt {c : X → X → ℝ} {p₀ ν μ : ProbabilityMeasure X}
...
quantity. `integrable_cost` is likewise a genuine field, and a load-bearing one: it is what lets a
...
structure IsSinkhornPlan (c : X → X → ℝ) (κ : ℝ) (p₀ νref μ : ProbabilityMeasure X) (b : ℝ)
  ...
  integrable_cost : Integrable (fun z : X × X => c z.1 z.2) (γ : Measure (X × X))
...
theorem sinkhornObjectiveENN_prodCoupling (c : X → X → ℝ) (κ : ℝ) (p₀ ν : ProbabilityMeasure X) :
...
theorem mem_sinkhornBall_reference (c : X → X → ℝ) (hc : ∀ x y, 0 ≤ c x y)
    (hcm : Measurable fun z : X × X => c z.1 z.2) (p₀ ν : ProbabilityMeasure X) (κ ε : ℝ)
    (hint : Integrable (fun z : X × X => c z.1 z.2) ((prodCoupling p₀ ν : Measure (X × X))))
...
infimum, so any coupling near the infimum is automatically finite in both summands. That finiteness
...
theorem exists_isSinkhornPlan_of_mem_sinkhornBall (c : X → X → ℝ) (hc : ∀ x y, 0 ≤ c x y)
    (hcm : Measurable fun z : X × X => c z.1 z.2)
    (p₀ ν μ : ProbabilityMeasure X) (κ ε η : ℝ) (hκ : 0 < κ) (hη : 0 < η)
    (hμ : μ ∈ sinkhornBall c p₀ ν κ ε) :
    ∃ γ : ProbabilityMeasure (X × X), IsSinkhornPlan c κ p₀ ν μ (ε + η) γ := by
