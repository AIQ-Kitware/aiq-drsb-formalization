...
| `Drsb.sdrsb_cost_bound` | ✅ **PROVED** (dependency-clean, **attainment-free**) | — | **The SDRSB evaluation-card claim, discharged.** Composes the proved `WangGaoXie2023.sinkhorn_weak_duality_kernel` (per-point DV bound integrated over `p₀`) with a *near-optimal* budget (`≤ ε + η`) + `η ↓ 0`, under `hκ>0` + the **disintegration-only** edge `hSink : ∀ η>0, IsSinkhornWitness … (ε+η)`. The attainment content is gone (same `sInf` argument as WDRSB); what remains is `condKernel` disintegration + KL chain rule + regularity, now named `Drsb.IsSinkhornWitness`. `sdrsb_cost_bound_of_attained_witness` is the receipt that the old attained edge implies the new one. Dual restricted to `0<lam` (the `lam=0` `logPartition` is junk — ess-sup convention unencoded). Ball uses the audit-fixed external `ν`. |
