...
The original is not an arXiv source and is written in older French analytic language.  The goal here is not to compress the idea, but to keep the proof architecture and translate the symbols into a modern form that can be compared with the finite Sinkhorn and continuum Schrodinger-bridge formalization.
...
\item The fixed-point operator $Q$ is the continuum analogue of composing the two Sinkhorn quotient updates.
\item The mass identity \eqref{eq:fortet-mass-identity} is a continuum version of the finite identity that the current rescaled coupling has the prescribed total mass.  It is the main device that kills residual scaling in one-sided comparison arguments.
\item Fortet's proof is not the Hilbert-metric contraction proof used for the finite projective-lag seam.  It is useful for existence/uniqueness and for understanding why normalization identities are load-bearing.
\item A Lean formalization of Fortet directly would require measurable kernels, monotone convergence, almost-everywhere quotient conventions, and local uniform convergence classes.  For the current finite proof, translate only the algebraic skeleton: denominator map $G$, update map $Q$, mass identity, max/min comparison.
