...
\path{prose/papers/LemmensNussbaum2013_hilbert_metric_survey.pdf}.

\section*{Purpose of this note}
The earlier note only summarized the survey.  This replacement keeps the modern notation and proof-level details that are useful for formalization: the order definition of Hilbert's metric, the proof of its basic metric properties, the equivalence with Hilbert's cross-ratio metric, nonexpansiveness of order-preserving homogeneous maps, the Birkhoff contraction theorem as the main theorem seam, the finite positive-matrix diameter formula, the Perron--Frobenius operator application, and the special-cone formulas for simplicial, polyhedral, and symmetric cones.

The survey also contains later dynamical results for nonexpansive maps.  Those are recorded as theorem seams rather than fully reproved, because the survey itself cites external sources for several of them.  The fully expanded material below covers the parts whose proofs are present in the paper and most directly relevant to Birkhoff--Hopf/Sinkhorn formalization.
...
\section{Later dynamics results recorded as theorem seams}
The survey's final section concerns nonexpansive maps on Hilbert geometries, especially normalized order-preserving homogeneous maps
\[
  g(x)=\frac{f(x)}{\varphi(f(x))}
\]
on a base \(\Sigma_\varphi^\circ\).  The important formalization lesson is that order-preserving homogeneous maps are automatically Hilbert-nonexpansive, while finite projective diameter gives strict contraction.

The survey records the following high-level results:
\begin{enumerate}[leftmargin=2em]
\item Nussbaum-type bounds on periods of periodic orbits for nonexpansive maps on finite-dimensional Hilbert geometries.
\item Denjoy--Wolff type alternatives for fixed-point-free nonexpansive maps: orbits converge toward boundary faces or horofunctions under additional geometric hypotheses.
\item For polytopal Hilbert geometries, the isometric embedding into \(\ell_\infty^N\) converts nonexpansive dynamics into finite-dimensional normed-space dynamics.
\end{enumerate}
These are not the main dependencies for finite Sinkhorn scaling, but they justify using Hilbert metric as a general nonlinear Perron--Frobenius tool.

\section{Lean comparison targets}
The core finite-dimensional statements to carry into Lean are:
\begin{enumerate}[leftmargin=2em]
