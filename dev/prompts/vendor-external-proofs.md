# Agent prompt — vendor an external proof and use it to push a DRSB `sorry` forward
...
one of the remaining `sorry`s** — a real, honest step, no vacuous scaffolds.
...
1. **`mrdouglasny/gibbs-variational` — `GibbsVariational/GaussianEntropy.lean`**
   — **Apache-2.0** (trivially compatible), Mathlib-only, ~1 `sorry` in the repo (not in
...
3. **`raphaelrrcoelho/formal-mathfin`** — **Apache-2.0**, ~45k lines, build-enforced
   axiom-clean. Full continuous Itô/SDE-existence(1-D)/Girsanov(BS)/Feynman-Kac +
...
If you judge target 2 (flow-sinkhorn → a data-driven worst-case sorry) more tractable,
...
- A full `lake build` is green at the end; any remaining `sorry` is *strictly fewer or
  strictly more isolated* than before (a bare sorry → an explicit named hypothesis counts
